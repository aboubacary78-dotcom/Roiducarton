...
/** Joue le bruitage d'un événement. Respecte le réglage « muet ». */
export function playEventSfx(text: string): EventSfx {
  const id = sfxFor(text);
  if (!isMuted()) { try { BANK[id](); } catch { /* silent */ } }
  return id;
}
