# Pack 14 · Les onze morts, en comédie (11 images)

## 📦 Contexte projet (à lire en entier · tu pars de zéro, sans accès au code)

Tu travailles sur **« Le Roi du Carton »**, un jeu mobile de survie : un SDF
survit jour après jour dans une ville française. Direction artistique
**« Carton Craft »** : chaque illustration est un **diorama miniature en carton
kraft fait main**, comme une maquette artisanale photographiée.

## ⚠️ RÈGLE N°1 · L'HUMOUR EST LA SIGNATURE DU JEU

« Le Roi du Carton » est une COMÉDIE noire, et TOUTE sa direction artistique
repose là-dessus : on rit de la misère avec tendresse, on ne s'y vautre jamais.
**CHAQUE image doit faire sourire** : une situation absurde, un gag visuel, une
expression comique du personnage. Une image simplement triste, plate ou
déprimante est HORS SUJET et sera refusée, même si elle est techniquement
réussie.

**Style OBLIGATOIRE pour chaque image :**
- diorama en carton kraft, textures carton ondulé et papier découpé visibles ;
- le héros : SDF en carton découpé, barbe grise, manteau rapiécé, visage
  dessiné au feutre ;
- lumière chaude d'atelier, petite guirlande lumineuse en fond ;
- **HUMOUR NOIR : chaque image doit être DRÔLE**, jamais triste ni
  misérabiliste ; le héros a une expression très marquée et expressive ;
- **format paysage 3:2, AUCUN texte ni lettre dans l'image.**

**Des images de référence te sont fournies en pièces jointes** avec ce brief :
observe-les attentivement et imite exactement ce style (matière, lumière,
personnage). Toute image qui s'en écarte sera refusée.

## 🚚 Protocole de livraison

1. **Noms de fichiers EXACTS**, copiés depuis la liste finale.
2. **VRAIS fichiers .webp** : un PNG renommé en .webp ne compte pas. Si tu ne
   sais pas encoder en webp, livre des **.png honnêtes** (extension .png),
   max 1600 px de large : la conversion sera faite à l'intégration.
3. **Poids raisonnable** : ≤ 1200 px de large, objectif < 200 Ko par image.
4. **Génère UNIQUEMENT les onze fichiers de la liste.** Rien d'autre.
5. **Livraison : un fichier ZIP** contenant un dossier `assets/`.

---

## 🎯 Ce pack remplace des images existantes

Les sept premières images de cette liste **existent déjà et sont refusées**.
Elles sont techniquement très réussies et ratent complètement leur cible :
elles sont tristes. Un homme la tête dans les mains dans une ruelle sombre, un
corps effondré sur un trottoir, c'est du misérabilisme, et le jeu ne fait pas
ça. Deux d'entre elles portent en plus du texte peint dans le décor, ce que le
protocole interdit.

Il n'y a **aucune indulgence à avoir sur ce point** : si une image de ce pack
peut être décrite comme « émouvante » ou « poignante » sans qu'on ait envie de
sourire, elle est ratée. On veut le registre du dessin animé muet : Buster
Keaton, Tex Avery, Wallace et Gromit. La mort y est une contrariété, pas un
drame.

---

## 🎬 LES DEUX FILS CONDUCTEURS DU PACK

Ces onze images s'affichent au même endroit, la une d'un journal, à la fin
d'une partie, et doivent se lire comme **une série**. Deux personnages
récurrents assurent cette unité, et ce sont eux qui portent la comédie.

### 1. LE FANTÔME EN CARTON

**Le héros meurt, et son fantôme reste.** C'est une petite silhouette du même
personnage, découpée dans du **papier calque froissé ou du carton très clair**,
flottant à trente centimètres au-dessus du corps, légèrement transparente, avec
le **même visage au feutre**, mais une expression totalement différente de
celle du corps.

C'est lui qui fait rire. Le corps est mort et immobile ; le fantôme, lui,
**commente**. Il s'agace, il applaudit, il pointe du doigt, il regarde sa
montre. Dans chaque image de ce pack, le fantôme a une réaction précise à ce
qui vient d'arriver, et c'est cette réaction qui EST la blague.

**Il doit être présent et lisible dans les onze images.**

### 2. LE PIGEON

Il est déjà partout dans le jeu. Ici il joue le témoin impassible : il ne
s'émeut de rien, il en profite, ou il rend un hommage d'un sérieux
disproportionné. Un ou plusieurs selon les scènes.

### Ce que ces deux-là permettent

Le corps peut être franchement mort, yeux en croix, langue de travers, pose
absurde, sans que l'image devienne glauque, parce que le fantôme et le pigeon
tiennent le registre comique. C'est exactement le mécanisme du dessin animé.

---

## 📐 Contrainte d'affichage

Ces images s'affichent dans **la une d'un journal**, en photo de presse,
**haute de 144 px sur un téléphone**, avec un gros titre au-dessus. Donc :

- **sujet principal net et central**, gros dans le cadre ;
- le corps et le fantôme doivent être identifiables **en un coup d'œil** ;
- **rien d'important dans les coins** : un tampon « 💀 » est incrusté en bas à
  droite à l'affichage ;
- **AUCUN texte, aucune lettre, aucun panneau écrit.** Les images actuelles en
  ont, c'est l'une des raisons de leur refus.

---

# LES SEPT CAUSES DE MORT (remplacements)

### 1. `death-despair.webp` · Le Moral à Zéro

**Ce que le jeu dit :** « L'esprit a lâché avant le corps. »

**Le gag, on prend la phrase au pied de la lettre.** Le corps se porte
parfaitement bien : le héros est **debout, en pleine forme**, l'air surpris, en
train de se palper le torse et de regarder autour de lui, il cherche quelque
chose. Ce qu'il cherche est assis à deux mètres, sur une caisse : **son propre
fantôme**, avachi, bras croisés, boudeur, qui refuse catégoriquement de
remonter dedans. Le fantôme fait la tête ; le corps ne comprend pas.

**Le pigeon :** posé entre les deux, il regarde alternativement l'un puis
l'autre, comme un arbitre de tennis.

**Ambiance :** ruelle en début de soirée, guirlande allumée au-dessus d'eux.

---

### 2. `death-hunger.webp` · Le Ventre Vide

**Ce que le jeu dit :** « Mort de faim dans une ville pleine de restaurants. »

**Le gag :** le héros est étendu sur le dos juste devant **la vitrine éclairée
d'un restaurant en carton**, débordante de plats découpés dans du carton. Il a
noué une serviette autour du cou et tient encore une fourchette en l'air, bien
droite, à bout de bras. Son fantôme, lui, a **traversé la vitre** et se tient à
l'intérieur, penché au-dessus d'une assiette, en train d'essayer désespérément
de piquer un morceau, sa fourchette fantôme passe au travers à chaque fois. Il
est furieux.

**Le pigeon :** à l'intérieur du restaurant lui aussi, attablé, une serviette
au cou, en train de manger tranquillement. Il a réussi, lui.

**Ambiance :** nuit, la vitrine est la seule source de lumière chaude, elle
éclaire le corps par en dessous.

---

### 3. `death-thirst.webp` · La Grande Soif

**Ce que le jeu dit :** « Assoiffé au pays des fontaines publiques. Elles
étaient toutes en travaux. »

**Le gag, le timing.** Le héros est effondré au pied d'une fontaine en carton,
la bouche grande ouverte tournée vers le bec, la langue pendante, dans la pose
du naufragé. Et **la fontaine vient de se remettre en marche** : un jet
puissant, magnifique, en papier de soie bleu, qui arrose tout, juste à côté de
sa bouche. Trente secondes trop tard. Son fantôme, au-dessus, a les deux bras
levés au ciel dans un « MAIS ENFIN » muet et parfaitement lisible.

**Le pigeon :** il prend sa douche dans le jet, l'air enchanté.

**Ambiance :** plein soleil de midi, ombres courtes, guirlande éteinte sur un
lampadaire.

---

### 4. `death-exhaustion.webp` · La Dernière Sieste

**Ce que le jeu dit :** « Il voulait juste dormir un peu. Le corps a pris ça au
pied de la lettre. »

**Le gag :** le héros est allongé sur un banc en carton dans la position la plus
confortable qu'il ait jamais trouvée, un vrai sourire de béatitude sur le
visage, les mains croisées sur le ventre, un pied qui dépasse. Il n'a jamais
été aussi bien. Son fantôme, au-dessus, essaie de le réveiller : il le secoue
par l'épaule, il tape dans ses mains, il lui souffle dans l'oreille. Rien. Le
fantôme est épuisé, ce qui est un comble.

**Le détail absurde :** un réveil en carton est tombé au sol et s'est cassé en
deux, les aiguilles arrêtées.

**Le pigeon :** installé sur le ventre du héros, endormi lui aussi.

**Ambiance :** petit matin bleuté, une seule guirlande encore allumée dans un
arbre au-dessus du banc.

---

### 5. `death-cold.webp` · La Nuit Glaciale

**Ce que le jeu dit :** « La rue est glaciale avec ses rois. »

**Le gag :** le héros a **gelé debout, en pleine action**, comme une statue.
Bras tendu, index levé, bouche ouverte : la posture exacte de quelqu'un qui
hèle un taxi. Il est entièrement recouvert de givre (paillettes blanches sur le
carton) et parfaitement immobile depuis manifestement plusieurs heures.

**Le pigeon :** deux pigeons se sont perchés sur son bras tendu comme sur une
branche, et un troisième s'est installé sur sa tête. Ils sont là depuis un
moment.

**Le fantôme :** il grelotte, lui aussi, ce qui n'a aucun sens et c'est justement
la blague, il se frotte les bras en claquant des dents.

**Ambiance :** nuit bleue très froide, halo d'un lampadaire, guirlande dont les
ampoules sont prises dans la glace.

---

### 6. `death-injury.webp` · Trop de Coups

**Ce que le jeu dit :** « Trop de coups, pas assez de pansements. L'arithmétique
de la rue. »

**Le gag, l'arithmétique, littéralement.** Le héros est allongé sur le dos,
recouvert de la tête aux pieds de **dizaines de petits pansements en carton**,
collés dans tous les sens, superposés, y compris sur son manteau et sur une de
ses chaussures. Il en est entièrement tapissé. Et à trente centimètres de sa
main tendue, hors de portée : **un dernier pansement**, encore dans son
emballage, celui qui aurait suffi.

**Le fantôme :** il désigne ce pansement du doigt d'un air accusateur, en
regardant droit vers nous, comme pour prendre le spectateur à témoin.

**Le pigeon :** il tient l'emballage dans son bec et s'apprête à s'envoler avec.

**Ambiance :** fin d'après-midi, ruelle, guirlande accrochée à une gouttière.

---

### 7. `death-combat.webp` · Tombé au combat

**Ce que le jeu dit :** la mort dans une rixe, quel que soit l'adversaire.

**Le gag, la disproportion.** Le héros est étalé de tout son long sur le dos,
bras en croix, yeux en croix, langue de travers, complètement K.O. Et le
vainqueur, debout sur sa poitrine dans une pose de champion du monde, une aile
levée en signe de victoire : **un pigeon**. Un seul. Tout petit. Il porte une
minuscule ceinture de champion en carton doré.

**Le fantôme :** beau joueur, il applaudit poliment. Il a l'air sincèrement
impressionné.

**Le détail :** deux autres pigeons font office de public, l'un tient un
minuscule drapeau.

**Ambiance :** ruelle en fin de journée, poussière soulevée par le combat encore
en suspension dans la lumière.

---

# LES QUATRE FINS PARTICULIÈRES

Le jeu reconnaît quatre morts remarquables, qui remplacent la photo générique
quand elles se produisent. Ce sont les plus savoureuses, et celles que le
joueur a envie de montrer.

### 8. `death-jour-1.webp` · Le Speedrun

**Ce que le jeu dit :** « Mort le premier jour. Un record, dans un sens. »

**Le gag :** **il n'a même pas eu le temps de s'installer.** Le carton qui
devait lui servir de maison est encore **plat, plié, avec son ruban adhésif de
livraison intact**, appuyé contre un mur. À côté, une petite valise en carton
encore fermée. Et lui, allongé par terre, toujours dans son manteau boutonné,
dans la position de quelqu'un qui vient tout juste d'arriver et qui posait ses
affaires.

**Le fantôme :** il est déjà debout, valise fantôme à la main, en train de
**regarder sa montre** avec l'air agacé de quelqu'un dont le train est en
avance.

**Le pigeon :** il arrive en volant avec une petite banderole de bienvenue en
carton, trop tard, évidemment.

**Ambiance :** grand jour, lumière franche, tout est neuf et propre, ce qui
rend la scène encore plus absurde.

---

### 9. `death-riche.webp` · Riche et Mort Quand Même

**Ce que le jeu dit :** « Parti avec plus de 30 € en poche. L'argent ne fait pas
la survie. »

**Le gag :** il est mort **en position de milliardaire**. Étendu sur un petit
monticule de pièces en carton comme sur une chaise longue, une main derrière la
tête, les jambes croisées, des lunettes de soleil en carton sur le nez et un
tube de carton en guise de cigare au coin de la bouche. Il a l'air
extraordinairement satisfait. Il est mort, mais il est mort riche.

**Le fantôme :** il essaie de **prendre les pièces avec lui** et elles lui
passent au travers des mains, une par une. Il s'acharne. C'est la seule chose
qui l'intéresse.

**Le pigeon :** il en emporte une dans son bec, sans se presser.

**Ambiance :** ruelle la nuit, les pièces attrapent la lumière chaude de la
guirlande et brillent beaucoup trop pour ce qu'elles valent.

---

### 10. `death-canicule.webp` · Cuit par la Canicule

**Ce que le jeu dit :** « Le bitume était une plancha. Vous étiez dessus. »

**Le gag :** il est étalé face contre le trottoir, bras et jambes écartés,
littéralement cuit. Et **juste à côté de lui, sur le bitume, un œuf est en train
de frire**, blanc pris et jaune bien rond, dans une petite auréole de graisse.
Le trottoir fume légèrement.

**Le pigeon :** il mange l'œuf. Avec appétit.

**Le fantôme :** il **s'évente** avec un morceau de carton, en sueur, et son
contour ondule comme l'air au-dessus d'une route surchauffée. Un fantôme qui a
trop chaud, c'est absurde, et c'est la blague.

**Ambiance :** midi écrasant, lumière blanche et dure, ombres très courtes,
guirlande qui pend molle et éteinte.

---

### 11. `death-doyen.webp` · Le Doyen

**Ce que le jeu dit :** « Tombé après 10 jours de règne. Les légendes aussi
finissent en carton. »

**Le gag, les honneurs, à l'échelle du carton.** C'est un enterrement d'État
en miniature, joué entièrement par des pigeons, et d'un sérieux total. Le héros
est allongé sur un brancard fait de deux planches et d'un vieux tapis, les
mains croisées sur la poitrine, un air de dignité immense. Sur un coussin en
carton posé sur son ventre : **la couronne en carton**, celle du titre du jeu.

**Les pigeons :** une haie d'honneur de six pigeons sur deux rangs, parfaitement
alignés. L'un d'eux joue d'une minuscule trompette en carton, un autre porte un
drapeau en berne.

**Le gag dans le gag :** au fond, à peine visible et flou, **un pigeon plus
petit est déjà en train d'essayer la couronne** sur sa propre tête pour voir si
elle lui va. La relève ne perd pas de temps.

**Le fantôme :** au-dessus, il fait un salut militaire impeccable, très ému,
très fier. C'est la seule image du pack où il ne râle pas.

**Ambiance :** crépuscule doré, toutes les guirlandes du quartier allumées d'un
coup, comme pour lui.

---

## Récapitulatif des noms de fichiers

```
death-despair.webp
death-hunger.webp
death-thirst.webp
death-exhaustion.webp
death-cold.webp
death-injury.webp
death-combat.webp
death-jour-1.webp
death-riche.webp
death-canicule.webp
death-doyen.webp
```

Onze fichiers. Pas un de plus.

**Rappel final, le plus important :** le fantôme en carton doit être présent et
lisible dans les onze. C'est lui qui transforme une série d'images tristes en
une série d'images drôles, et c'est la seule chose qui manquait aux versions
précédentes.
